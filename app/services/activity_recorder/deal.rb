...
module ActivityRecorder
  class Deal < ApplicationService
    ...
    def initialize(deal, user, params)
      @params = params
      @deal = deal
      @user = user
    end
    ...
    def call
      ...
      valuation_phase_change if deal.funding_round.present?
    ...
    def record_target_change
      return if params[:target].blank? || params[:target].to_f == deal.target

      update_valuation_multiple(params[:target], deal.target) if deal.property?
      record_activity('target', deal.target, params[:target], Activity::actions[:deal_valuation_changed])
    end
    ...
    def record_valuation_change
      ...
      record_activity('valuation', deal.funding_round.valuation, valuation, Activity::actions[:deal_valuation_changed])
    end

    def valuation_phase_change
      return if params[:funding_round_attributes].blank?

      valuation_phase = Option.find_by(id: params[:funding_round_attributes][:valuation_phase_id]).statement
      record_activity('valuation_phase', deal.valuation_phase, valuation_phase, Activity::actions[:valuation_phase_changed])
    end
    ...
    def record_rental_change
      ...
      record_activity('rental_amount', deal.property_detail.rental_amount, rental_amount, Activity::actions[:rental_amount_changed])
    end

    def record_rental_cycle_change
      if params[:property_detail_attributes].blank? || params[:property_detail_attributes][:rental_period_id].blank?
        return
      end

      rental_duration = Option.find_by(id: params[:property_detail_attributes][:rental_period_id]).statement
      record_activity('rental_duration', deal.rental_duration, rental_duration, Activity::actions[:rental_cycle_changed])
    end

    def update_valuation_multiple(new_value, old_value)
      multiple = new_value.to_f / old_value
      multiple = deal.valuation_multiple.to_f.positive? ? deal.valuation_multiple.to_f * multiple : multiple
      deal.update!(valuation_multiple: multiple)
    end

    def record_activity(field_name, old_value, new_value, action)
      activity = Activity.new(field_name:,
                              new_value:,
                              old_value:,
                              action:)
      activity.user = user
