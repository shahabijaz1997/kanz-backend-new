...
class Deal < ApplicationRecord
  ...
  def start_date_in_future
    ...
    errors.add(:base, I18n.t('deals.start_date_in_future'))
  end

  def start_date_and_end_date
    return if start_at.blank? || end_at.blank?
    return if start_at < end_at
    errors.add(:base, I18n.t('deals.end_date_after_start'))
  end

  def start_and_end_date_presence
    return if draft?
    return if start_at.present? && end_at.present?
    errors.add(:base, I18n.t('deals.dates_presence'))
  end
  ...
  def validate_status_change
    if status == 'submitted' && !(status_was.in? %w[draft reopened])
      errors[:base] << I18n.t('deals.submitted_condition')
    elsif status == 'verified' && status_was != 'submitted'
      errors[:base] << I18n.t('deals.verified_condition')
    elsif status == 'reopened' && !(status_was.in? %w[verified submitted])
      errors[:base] << I18n.t('deals.reopened_condition')
    elsif status == 'approved' && status_was != 'verified'
      errors[:base] << I18n.t('deals.approved_condition')
    elsif status == 'live' && status_was != 'approved'
      errors[:base] << I18n.t('deals.live_condition')
    end
  end
